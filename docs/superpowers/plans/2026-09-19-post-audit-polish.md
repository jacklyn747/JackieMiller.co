...
1. **Task 2 deviated from this plan's literal CSS.** The plan's Step 2 CSS block for `.hm-hero__portrait` did not include a `max-width`. Task 2's own implementation added `max-width: 500px` (matching the old `.hm-flash` illustration's cap, per the rule's own comment in `home.css`) because `.hm-hero__art` has no width of its own — without the cap the photo stretched edge-to-edge in the single-column mobile layout instead of reading as a proportionate portrait. This was a well-reasoned fix for the container's actual layout behavior at tablet widths, already caught and approved in Task 2's own review, but never recorded at the plan level until now.
2. **The final whole-branch review found, and this same fix pass corrected, three issues introduced across Tasks 2 and 4:**
   - A duplicated "governed pipeline" phrase closing two separate `<h2>` headlines (the pre-existing "4 Operating Pillars" section's "Four pillars, *one governed pipeline*" and the just-added "How I Work" section's "The craft decisions behind *every governed pipeline*"). The "How I Work" headline was changed to "...*every accelerated delivery*"; the Pillars headline was left untouched.
   - Identical `alt="Jackie Miller"` text on both the hero and About-teaser portrait `<Image>`s. Changed to `"Jackie Miller, portrait"` (hero) and `"Jackie Miller, seated portrait"` (About-teaser) so they're distinguishable to screen readers.
   - Inconsistent border treatment between the two portrait frames — `.hm-hero__portrait` (Task 2, new) had `border-radius: 18px` with no border, while `.hm-about__portrait` (pre-existing) had a `0.5px solid var(--ink-dim)` border with no radius. Brought `.hm-hero__portrait` to match `.hm-about__portrait`'s existing treatment (border, no radius) so the two frames read as a matched pair.
3. **Manual browser verification (Task 6) was actually performed by the controller directly in the session, not via a dispatched subagent** — covering: hero portrait render, About-teaser showing a distinct pose, Selected Work/How I Work new copy, Digital Literacy's Kirkpatrick line (verified via page text extraction), and one demonstration case study's (`technical-product-enablement`) new Artifact Summary callout. This should have been logged in `.superpowers/sdd/progress.md` and reported at the time it happened; noting it now for the record.
